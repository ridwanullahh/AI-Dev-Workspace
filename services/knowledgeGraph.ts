...
class KnowledgeGraphSystem {
  ...
  private async loadGraphData(): Promise<void> {
    try {
      // Load nodes
      const nodes = await storageService.getKnowledgeNodes();
      for (const node of nodes) {
        this.nodes.set(node.id, node);
      }

      // Load edges (stored as knowledge nodes with type 'edge')
      const edgeNodes = await storageService.getKnowledgeNodes(undefined, 'edge');
      for (const edgeNode of edgeNodes) {
        const edge = this.parseEdgeNode(edgeNode);
        if (edge) {
      ...
      // Load clusters
      const clusterNodes = await storageService.getKnowledgeNodes(undefined, 'cluster');
      for (const clusterNode of clusterNodes) {
        const cluster = this.parseClusterNode(clusterNode);
        if (cluster) {
  ...
  private parseEdgeNode(node: KnowledgeNode): GraphEdge | null {
    try {
      if (node.type !== 'edge' || !node.content) return null;
  ...
  private parseClusterNode(node: KnowledgeNode): GraphCluster | null {
    try {
      if (node.type !== 'cluster' || !node.content) return null;
  ...
  async addNode(
    ...
    // Save to storage
    await storageService.saveKnowledgeNode(node);
  ...
  async addEdge(
    ...
    // Update nodes
    await storageService.saveKnowledgeNode(sourceNode);
    await storageService.saveKnowledgeNode(targetNode);
  ...
  private async saveEdgeAsNode(edge: GraphEdge): Promise<void> {
    ...
    await storageService.saveKnowledgeNode(edgeNode);
  }
  ...
  private async saveClusterAsNode(cluster: GraphCluster): Promise<void> {
    ...
    await storageService.saveKnowledgeNode(clusterNode);
  }
  ...
  async exportGraph(projectId?: string): Promise<{
    ...
    if (projectId) {
      nodes = nodes.filter(node => node.metadata.projectId === projectId);
      edges = edges.filter(edge => {
        const sourceNode = this.nodes.get(edge.sourceId);
        const targetNode = this.nodes.get(edge.targetId);
        return sourceNode?.metadata.projectId === projectId || 
               targetNode?.metadata.projectId === projectId;
      });
      clusters = clusters.filter(cluster => 
        cluster.nodes.some(nodeId => {
          const node = this.nodes.get(nodeId);
          return node?.metadata.projectId === projectId;
        })
      );
    }
